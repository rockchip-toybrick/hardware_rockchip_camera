...
status_t GoogSensorWrapper::Enable() {
  ...
  (void)sensor_sampling_period_us_;
  Return<Result> result =
      event_queue_->enableSensor(handle_, 100000, 0);
  if (!result.isOk()) {
...
void GoogSensorWrapper::WaitForEvent(unsigned int timeout_ms) {
  {
  ...
  {
    std::unique_lock<std::mutex> l(event_buffer_lock_);
    if (timeout_ms) {
...
status_t GoogSensorWrapper::SetSampFreq(int freq) {
  status_t res = OK;
  std::lock_guard<std::mutex> l(event_queue_lock_);

  Return<Result> result =
      event_queue_->enableSensor(handle_, (int64_t)freq, 0);
  if (!result.isOk()) {
    ALOGE("%s %d enable sensor Hidl call failed: %s", __func__, __LINE__,
          result.description().c_str());
    return -1;
