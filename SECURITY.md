# Security Policy
## Supported Versions
The following versions of camerahal are currently supported with security updates:
| Version                      | Supported                      |
| :--------------------------- | :----------------------------- |
...
If you discover a security vulnerability in camerahal, please report it privately using the following methods:
### Reporting Channels
1. **Primary Contact** (Recommended)
   - Email the maintainer at: **toybrick@rock-chips.com**
   - Subject format: `[SECURITY] camerahal vulnerability report - [brief description]`
...
- **Affected Versions**: Specific camerahal version or Git commit hash
- **Affected Components**: Specific modules (e.g., ISP_driver/RGA_driver/dma-buf/IOMMU, etc.)
- **Environment Details**:
...
When using camerahal, the following security measures are recommended:
1. **Permission Control**: Restrict access to `/dev/video*` and `/dev/media*` device nodes with appropriate permissions
2. **Timely Updates**: Keep camerahal and the drm kernel driver updated to the latest version to receive the latest security fixes
...
View [GitHub Security Advisories](https://github.com/rockchip-toybrick/hardware_rockchip_camera/security/advisories) for publicly disclosed security announcements.
---
Thank you for contributing to camerahal security!